...
class PokemonCards
  ...
  def self.find_card(card_name, year_min = 1996, year_max = 2016)
    ...
    cards_array = cards.each_with_object([]) do |card, card_array|
      card_array << [card.name, card.image_url, PokemonCards.set_years[card.set]]
    end
  ...
  def self.set_years
    @set_years ||= Pokemon::Set.all.each_with_object({}) do |set, set_year_hash|
      set_year_hash[set.name] = set.release_date.split("/").last.to_i
    end
  end
